...
# Cek logs
docker logs -f otakudesu-api

# Stop container
docker stop otakudesu-api
```

Base image yang digunakan: `mcr.microsoft.com/playwright/python:v1.44.0-jammy`  
→ Chromium + semua dependensi OS sudah pre-installed, tidak perlu `playwright install` manual.

### Deploy ke Render / Koyeb

1. Push repo ke GitHub
2. Buat **Web Service** baru, pilih repo tersebut
3. Set **Runtime** ke `Docker`
4. Set **Port** ke `8000`
5. Deploy
...
| Library | Kegunaan |
|---------|----------|
| [FastAPI](https://fastapi.tiangolo.com/) | Web framework async |
| [Uvicorn](https://www.uvicorn.org/) | ASGI server |
| [Playwright](https://playwright.dev/python/) | Headless browser, bypass Cloudflare |
...
- Setiap request membuka satu instance Chromium baru → response time ~3–8 detik per request (normal).
- Jika Otakudesu mengubah struktur DOM-nya, cukup update fungsi di folder `parsers/` tanpa menyentuh routes atau utils.
- Untuk caching response guna mempercepat API, bisa tambahkan [Redis](https://redis.io/) + [fastapi-cache2](https://github.com/long2ice/fastapi-cache) di atas `fetch_html()`.
